 ...
 * /api/pdf-to-ppt:
 *   post:
 *     summary: Convierte archivos PDF a presentaciones PowerPoint (.pptx)
 *     tags: [PDF → Office]
 *     requestBody:
 *       required: true
 *       content:
 ...
 *                 fileId:
 *                   type: string
 *                   description: ID para descargar el PPTX desde /api/download/:fileId
 ...
 *                 size:
 *                   type: number
 *                   description: Tamaño del PPTX en bytes
 *       400:
...
router.post('/', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const fileId = await fileStore.storeFile(
      pptxBuffer,
      originalName.replace(/\.pdf$/i, '.pptx'),
    ...
    res.json({
      success: true,
      fileId,
      fileName: originalName.replace(/\.pdf$/i, '.pptx'),

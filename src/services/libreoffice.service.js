...
class LibreOfficeService {
  ...
  async pdfToWord(inputPath, outputDir) {
    ...
    await execAsync(
      `libreoffice --headless --convert-to docx --infilter="writer_pdf_import" --outdir "${outputDir}" "${inputPath}"`
    );
  ...
  async pdfToExcel(inputPath, outputDir) {
    const filename = path.basename(inputPath, '.pdf');
    await execAsync(
      `libreoffice --headless --convert-to xlsx --infilter="calc_pdf_import" --outdir "${outputDir}" "${inputPath}"`
    );
  ...
  async pdfToPpt(inputPath, outputDir) {
    const filename = path.basename(inputPath, '.pdf');
    await execAsync(
      `libreoffice --headless --convert-to pptx --infilter="impress_pdf_import" --outdir "${outputDir}" "${inputPath}"`
    );
